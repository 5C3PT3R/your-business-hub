-- Update contacts table to add missing fields from PRD
-- Migration: 20260112_update_contacts_table

-- Add lifecycle stage enum
...
ALTER TABLE contacts
  ...
  ADD COLUMN IF NOT EXISTS data_completeness INTEGER DEFAULT 0 CHECK (data_completeness >= 0 AND data_completeness <= 100),
  ADD COLUMN IF NOT EXISTS custom_fields JSONB DEFAULT '{}'::jsonb,
  ADD COLUMN IF NOT EXISTS tags TEXT[] DEFAULT '{}',
  ADD COLUMN IF NOT EXISTS notes TEXT,
  ADD COLUMN IF NOT EXISTS last_activity_at TIMESTAMP WITH TIME ZONE;

-- Migrate existing name field to first_name
UPDATE contacts
...
-- Trigger to auto-update data completeness
...
-- Create view for contact list with computed fields
